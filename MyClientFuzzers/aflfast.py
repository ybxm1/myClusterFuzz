...
class AFLFASTEngine(engine.Engine):
  @property
  def name(self):
    return 'AFLFAST'

  def fuzz(self, input, output, info, target, aflfast_path, runtime, surplusum, nodename, execname):
    info_path = info + nodename + "_" + str(surplusum) + ".log"  # 日志名是啥问题不大，但是要避免同一个节点多次运行日志的时候出现日志覆盖的问题
    start_cmd = aflfast_path + " -i " + input + " -o " + output + " -m none " + target
    print(start_cmd)
    t_start = time.time()
    ...
    for proc in psutil.process_iter():  # 通过进程名的方式来kill进程
      if proc.name() == execname:  # 不同的模糊测试实例该名字不一样
        proc.terminate()
        print("aflfast process end successfully!")
    logger.info("任务完成！")
