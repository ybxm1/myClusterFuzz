      ...
      <el-form ref="form" :model="form" label-width="80px"> <!-- border-radius="12px" 只对div元素才有用 -->
            <el-form-item>
              <div style="display:inline-block;width:55%;margin-left:170px;">
                <font size="6" color="#009deb">混合模糊测试</font>
              </div>
            </el-form-item>
            <el-form-item>
            ...
            <el-form-item>
              ...
              <div style="display:inline-block;width:70%;margin-left:18px;">
                <font><strong>AFLFAST + QSYM</strong></font>
              </div>
            </el-form-item>
            <el-form-item>
              <div style="display:inline-block;width:25%;">
...
export default {
  ...
  computed: {  // 会实时计算吗？就是当在state中将CHANGE_CREA_DIALOG的变量值修改后，这里会实时接收吗？这里是否是一个处于等待状态的线程？
    canOpen: {
      get() {
        return this.$store.state.workstation.createMixfuzz;
      },
      set(value) {
        this.$store.commit("CHANGE_CREA_DIALOG_MIX", value);
      }
    }
  },
    ...
    createjob(){
      ...
      uplodaJobInfo(formData)
        .then(resp => {
          ...
          this.$store.commit("CHANGE_CREA_DIALOG_MIX", false);
