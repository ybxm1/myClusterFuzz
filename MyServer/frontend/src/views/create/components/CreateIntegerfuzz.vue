      ...
      <el-form ref="form" :model="form" label-width="80px"> <!-- border-radius="12px" 只对div元素才有用 -->
            <el-form-item>
              <div style="display:inline-block;width:55%;margin-left:170px;">
                <font size="6" color="#009deb">集成模糊测试</font>
              </div>
            </el-form-item>
            <el-form-item>
            ...
            <el-form-item>
              <div style="display:inline-block;width:25%;">
                <font>可执行文件名</font> <!-- afl-aflfast-libfuzz-honggfuzz 依次命名，然后各个节点依据其测试任务（surplusnum）来选择对应的可执行文件名 -->
              </div>
              <div style="display:inline-block;width:70%;">
...
export default {
  ...
  computed: {  // 会实时计算吗？就是当在state中将CHANGE_CREA_DIALOG的变量值修改后，这里会实时接收吗？这里是否是一个处于等待状态的线程？
    canOpen: {
      get() {
        return this.$store.state.workstation.createIntegerfuzz;
      },
      set(value) {
        this.$store.commit("CHANGE_CREA_DIALOG_INTEGER", value);
      }
    }
  },
